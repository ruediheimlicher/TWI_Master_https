...
static volatile uint8_t TWITASK=0; // aktuelle Task mit TWI
...
volatile uint8_t sevensegcode[] = {0x7E,0x30,0x6D,0x79,0x33,0x5B,0x5F,0x70,0x7F,0x7B,0x77,0x1F,0x4E,0x3D,0x4F,0x47};
...
volatile uint8_t twi_Call_count0=0;
volatile uint8_t twi_Reply_count0=0;
volatile uint8_t twi_Stat_count=0; //   Stat_count zurücksetzen
volatile uint8_t UhrLesen=0;
volatile uint8_t SPI_Call_count0=0;
...
//	OSZIALO;
	...
	*/
   https://www.mikrocontroller.net/articles/AVR-GCC-Tutorial/Die_Timer_und_Zähler_des_AVR#CTC_Clear_Timer_on_Compare_Match_.28Auto_Reload.29
...
void sevenseginit(void)
{
   ...
   DDRB |= 0x0F; // PORTB 0-3 Ausgang fuer siebenseg
   PORTB |= 0x0F; // hi
...
else
{
   ...
   uint8_t sega = zahl & 0x0F; // 4 bit
   uint8_t segb = (zahl & 0xF0)>>4; // bit 4-7. bit 7 ist enable
   err_gotoxy(0,2);
   err_puthex(zahl);   
   err_putc(' ');
   err_puthex(sega);  
   ...
   for(i=0; i<7; i++)
   {
      if (zahl & 0x80) // bit is hi 0x08 ist bit 7
      {
         if (i <4) // > PORTA
         {
            PORTA |= (1<<(i+1));
   ...
   PORTB |= segb;
   err_putc(' ');
   err_puthex(PINA&0x1E);

  // PORTA  = PINA & (sega < 1); // bit 0 ist besetzt
   //err_putc(' ');
   //err_puthex(PINA);
//   PORTB &= (segb & 0x07); 
}
 ...
 //  delay_ms(10);
   sei();
	while (1)
	{
      sevensegalt++;
     //PORTB ^= (1<<3);
   //   PORTA &= (sevensegalt & 0x01)<<4; // shift bit 0 auf PA4 
		...
		//	Startroutine noch im Gang: TWI beide Pins sind noch HI, also Rückwaertszaehlen
		...
		if (loopcount0 >= 0x8FFF)
		{
         ...
         if (startdelay)
         {
           ...
           // uint8_t sevenseghi = (sevenseg & 0xF0)>>4;
            //NSLog(@"A: %X B: %X C: %X",sevenseg,sevenseglo,sevenseghi);
      //      lcd_putc(sevenseglo);
       //     lcd_putc(sevenseghi);
        /*    
            uint8_t code = sevensegcode[sevenseg&0x0F];
            err_gotoxy(12,0);
            err_puthex(sevenseg&0x0F);
            err_putc(' ');
            err_puthex(code);
            sevensegout(sevensegcode[sevenseg&0x0F]);
...
#pragma mark PASSIVE
         ...
         if (spistatus &(1<<ACTIVE_BIT)) // CS beendet. ACTIVE_BIT ist NOCH gesetzt: Slave ist NEU passiv geworden. Aufraeumen, Daten uebernehmen
			{
            // ***********************
            /*
...
#pragma mark  MASTERTASK
      ...
      else // kein shift_in, mit TWI fortfahren
      {
         if ((spistatus & (1<<ACTIVE_BIT)) &&  (SPI_CONTROL_PORTPIN & (1<< SPI_CONTROL_CS_HC)))// SPI_CONTROL_CS_HC ist HI, aber ACTIVE_BIT wurde noch nicht zurueckgesetzt
         {
            /*
            spistatus &= ~(1<<ACTIVE_BIT);
            lcd_gotoxy(0,0);
